...
class Province:
    def __init__(self, terrain_type):
        ...
        self.owner = None        self.victory_points = random.randint(1, 5)
...
def main():
    ...
    while not check_victory(teams):
        for team in teams:
            player_interaction(team)
            production_phase(teams)
            Team.diplomacy_phase(teams)
            Team.espionage_phase(teams)
            for province in team1.provinces + team2.provinces + team3.provinces + team4.provinces:
                if province.owner:
                    for attacker in province.owner:
                        defending_team = None  # Implement logic to find the defending team
                        if defending_team:
                            defender = random.choice(defending_team)
                            combat(attacker, defender, province)
